...
SUPPORTED_MIMETYPES = ["application/vnd.google-apps." + ext for ext in  ["document", "presentation"]]


class Scraper:
    def __init__(self, supported_mimetypes = None):
        self._scopes = SCOPES
        self._supported_mimetypes = SUPPORTED_MIMETYPES if supported_mimetypes is None else supported_mimetypes
        self._credentials = self._get_auth()
        self._service = discovery.build('drive', 'v3', http=self._credentials.authorize(Http()))
    ...
    def get_file_text_content(self, file_id):
        return self._files_resource.export_media(fileId=file_id, mimeType="text/plain").execute()
    ...
    def get_permission_details(self, file_id, permission_id):
        return self._permissions_resource.get(fileId=file_id, permissionId=permission_id, fields='*').execute()
